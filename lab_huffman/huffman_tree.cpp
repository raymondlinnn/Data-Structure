                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (!singleQueue.empty() && mergeQueue.empty()) {
        HuffmanTree::TreeNode* temp_s = singleQueue.front();
        singleQueue.pop();
        return temp_s;
    }
    if (singleQueue.empty() && !mergeQueue.empty()) {
        HuffmanTree::TreeNode* temp_m = mergeQueue.front();
        mergeQueue.pop();
        return temp_m;
    }
    // normal condition
    HuffmanTree::TreeNode* temp_s = singleQueue.front();
    HuffmanTree::TreeNode* temp_m = mergeQueue.front();
    if (temp_s->freq.getFrequency() <= temp_m->freq.getFrequency()) {
        singleQueue.pop();
        return temp_s;
    }
    else{
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    queue<TreeNode*> mergeQueue;  // Queue containing the inner nodes
     ...
     for (auto it = frequencies.begin(); it != frequencies.end(); ++it) {
         HuffmanTree::TreeNode* temp = new HuffmanTree::TreeNode(*it);
         singleQueue.push(temp);
     }

    while(!(singleQueue.empty() && mergeQueue.size() == 1)){
        HuffmanTree::TreeNode* smaller = removeSmallest(singleQueue, mergeQueue);
        HuffmanTree::TreeNode* bigger = removeSmallest(singleQueue, mergeQueue);
        HuffmanTree::TreeNode* parent = new HuffmanTree::
        TreeNode(smaller->freq.getFrequency() + bigger->freq.getFrequency());
        parent->left = smaller;
        parent->right = bigger;
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        // when the while loop stops, current is a leaf node
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    */
    ...
    if (bfile.getNextBit() == 1) {
        ...
        new HuffmanTree::TreeNode(Frequency(bfile.getNextByte(), 0));
        return node;
    }else{
        HuffmanTree::TreeNode* node = new HuffmanTree::TreeNode(0);
