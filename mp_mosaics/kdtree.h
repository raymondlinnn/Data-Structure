...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode *root;
    size_t size;
    vector<Point<Dim>> saved; // should be sorted in the end
    ...
    double distance(const Point<Dim>& f, const Point<Dim>& s) const;
    // quickselect helper functions
    int partition(vector<Point<Dim>>& vec, int L, int R, int d);
    Point<Dim> KthSmallest(vector<Point<Dim>>& vec,int L, int R, int k, int d);

    // recursive helper function for constructor
    void build_helper(KDTreeNode*& subroot, vector<Point<Dim>>& saved, int d);

    void copy(KDTreeNode*& subroot, const KDTreeNode* other);

    void clear(KDTreeNode* subroot);

    // helper functions for find nearest neighbor
    Point<Dim> neighbor(const Point<Dim>& query, const KDTreeNode* subroot, int d);
    // check if need to update currentbest, if yes, update everything
    void replace(const Point<Dim>& query, Point<Dim>& currentBest,
