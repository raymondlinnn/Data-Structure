...
size_t insertion_idx(const std::vector<T>& v, const C& val)
{
    /* using linear search (the vector wouldn't be large)
    if (elements.empty())
        return 0;
...
*/
    ...
    // searching between [1, end_ - 1]
    while (start_ <= end_) {
        int m = (start_ + end_) / 2;
        if (v[m] == val || (v[m] > val && v[m-1] < val)){
            return m;
        }else if (v[m] < val){
            start_ = m + 1;
        }else{
    ...
    return -1; // actually it will never return -1
}
