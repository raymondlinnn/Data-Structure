...
{
    size_t f_idx = insertion_idx(subroot->elements, key);
     ...
     */
    // check the validity of first_larger_idx
    size_t size = subroot->elements.size();
    if (f_idx < size && subroot->elements[f_idx] == key){
        return subroot->elements[f_idx].value;
    }else{
        // when the key != key at the first_larger_idx, go to the corresponding child
        if (subroot->is_leaf)
            return V();
        else
            return find(subroot->children[f_idx], key);
    }
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    // if child is not a leaf, split children
    if (!new_left->is_leaf) {
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    size_t f_idx = insertion_idx(subroot->elements, pair);
    size_t size = subroot->elements.size();
    if (f_idx < size && subroot->elements[f_idx] == pair){
        return; // why don't we replace it?
    }else{
        // when the key != key at the first_larger_idx, go to the corresponding child
        if (!subroot->is_leaf) {
            insert(subroot->children[f_idx], pair);
            if (subroot->children[f_idx]->elements.size() >= order)
                split_child(subroot, f_idx);
        }
        else{
            // when the subroot is a leaf, we do the insert
            auto insert_pos = subroot->elements.begin() + f_idx;
            subroot->elements.insert(insert_pos, pair);
        }
    }
}
